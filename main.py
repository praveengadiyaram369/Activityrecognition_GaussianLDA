...
async def load_instance(request: Request, doc_id: int=Form(...)):
    print(doc_id)
    output = {'id':doc_id, 'X1': [doc_id, doc_id], 'Y1': [1,2], 'Z1': [1,2], 'X2': [1,2], 'Y2': [1,2], 'Z2': [1,2] }
    return templates.TemplateResponse('index.html', context={'request': request, 'options_list': test_samples_idx, 'instance_data':default_data, 'prev_instance_data':output,'output': output})
